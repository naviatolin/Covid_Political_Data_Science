...
#   labs(fill = "Cases") +
#   ggtitle("Reported COVID Cases 2020-10-17")
```
Looking at just MA
```{r}
library(gganimate)
...
```
```{r}
df_population_covid_political_with_map_with_color <- df_population_covid_political_with_map %>%
  mutate(
...
animation <- df_population_covid_political_with_map_with_color %>%
  filter(state_abbv == state_abbreviation ) %>%
...
df_population_covid_political_with_map %>%
  ...
  ggtitle("Political Affiliation per County in 2016")
```
